...
-- Mengambil token sesi aktif dari header HTTP (x-cbt-token atau Authorization Bearer)
...
-- Mengambil ID Pengguna aktif (mendukung Supabase Auth & CBT Session Token)
...
DECLARE
  ...
  v_user_id text;
BEGIN
  -- 1. Cek dari Supabase Auth bawaan jika ada
...
-- Mengambil Peran (ROLE) Pengguna Aktif ('ADMIN', 'TEACHER', 'STUDENT', atau 'ANON')
...
-- Mengambil ID Kelas untuk Siswa yang Sedang Login
...
-- Guru: Hanya dapat mengubah ujian buatannya sendiri
...
-- Guru: Hanya dapat menghapus ujian buatannya sendiri
...
-- SISWA: HANYA DAPAT MELIHAT UJIAN UNTUK KELASNYA YANG BERSTATUS AKTIF / TERJADWAL
...
-- Admin: Hak akses penuh ke seluruh bank soal
CREATE POLICY "questions_admin_all" ON public.lms_questions
  FOR ALL
  USING (public.get_current_cbt_user_role() = 'ADMIN');

-- Guru: Hak akses penuh pada soal dari ujian yang dibuat oleh guru tersebut
...
-- SISWA: HANYA DAPAT MELIHAT SOAL DARI UJIAN KELASNYA YANG SEDANG AKTIF
...
-- Admin: Hak akses penuh untuk monitoring, koreksi nilai, dan reset sesi
CREATE POLICY "attempts_admin_all" ON public.lms_attempts
  FOR ALL
  USING (public.get_current_cbt_user_role() = 'ADMIN');

-- Guru: Dapat memantau hasil/progres ujian buatannya
...
-- Guru: Dapat memberi nilai esai (grading) pada ujian buatannya
...
-- SISWA: HANYA DAPAT MELIHAT PENGERJAAN DIRINYA SENDIRI
...
-- SISWA: HANYA DAPAT MEMBUAT LEMBAR PENGERJAAN UNTUK DIRINYA PADA UJIAN KELASNYA
...
-- SISWA: HANYA DAPAT MENYIMPAN JAWABAN SELAMA STATUS MASIH 'IN_PROGRESS'
...
-- Guru: Dapat melihat daftar siswa & guru untuk rekap dan absensi
CREATE POLICY "users_teacher_select" ON public.lms_users
  FOR SELECT
  USING (public.get_current_cbt_user_role() = 'TEACHER');

-- Guru: Update profil diri sendiri
...
-- Siswa: Hanya dapat melihat akun diri sendiri
...
-- Siswa: Hanya dapat update akun diri sendiri (misal ubah kata sandi)
...
-- Anonim / Form Login: Izin membaca data dasar untuk verifikasi saat login
CREATE POLICY "users_anon_login" ON public.lms_users
  FOR SELECT
  USING (public.get_current_cbt_user_role() = 'ANON');

-- ------------------------------------------------------------------------------
-- 8. ATURAN KEAMANAN UNTUK MASTER DATA (KELAS, MAPEL, PENGATURAN, JADWAL)
-- ------------------------------------------------------------------------------

-- Master Kelas (lms_classes)
...
-- Master Mata Pelajaran (lms_subjects)
...
-- Jenis Penilaian (lms_assessment_types)
...
-- Pengaturan Sekolah (lms_settings)
...
-- Jadwal Pelajaran (lms_timetable)
...
-- Roster Guru & Beban Mengajar (lms_teacher_roster, lms_teacher_assignments)
...
-- ==============================================================================
-- SELESAI: SELURUH TABEL SEKARANG MEMILIKI PERLINDUNGAN ROW-LEVEL SECURITY (RLS)
-- ==============================================================================
